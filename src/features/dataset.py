...
def configure_for_performance(ds: tf.data.Dataset, batch_size: int = 16, shuffle: bool = True):
    ds = ds.cache()
    if shuffle:
...
def get_dataset(
    split: str,
    dataset_generator_type: ImageDatasetType = ImageDatasetType.BboxImagesDatasetGenerator,
    batch_size: int = 16,
    shuffle: bool = True
) -> tf.data.Dataset:
